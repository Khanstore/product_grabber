...
class ImportProductDuplicateSuggestion(models.TransientModel):
    ...
    _description = 'Import Product Duplicate Suggestion'
    _order = 'id'

    wizard_id = fields.Many2one(
        'import.product.from.website',
        string='Wizard',
        required=True,
        ondelete='cascade',
    )
    ...
    def action_update_product(self):
        ...
        wizard.selected_duplicate_id = self.product_id
        return wizard.update_existing_product()
...
class importProductFromWebsite(models.TransientModel):
    ...
    duplicate_suggestion_line_ids = fields.One2many(
        'import.product.duplicate.suggestion',
        'wizard_id',
        string='Duplicate Matches',
        readonly=True,
    )
    ...
    def _sync_duplicate_suggestion_lines(self):
        """Build the rich duplicate-match table shown on the import screen."""
        self.ensure_one()
        commands = [(5, 0, 0)]
        ...
        self.duplicate_suggestion_line_ids = commands
    ...
    def _scrape_source_url(self, interactive=True):
        ...
        self._sync_duplicate_suggestion_lines()
